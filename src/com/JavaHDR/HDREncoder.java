...
/**
 * Writes and reads default 32-bit_rle_rgbe HDR files, without any other fancy
 * stuff (no gamma, etc.)
 * 
 * Created by Victor Arellano (Ivelate). This code is licensed under the MIT
 * license - anybody can freely use or modify any part of this code and insert
 * any license into it without needing to attribute me for that. That's it!
 */
public class HDREncoder {
	...
	/**
	 * Writes the data in <img> into <file>, parsing the float values of <img> into
	 * an RGBE array
	 */
	public static void writeHDR(HDRImage img, File file) throws IOException {
		...
		if (!rgb)
			for (int s = 0; s < img.getInternalData().length; s += img.getWidth())
				for (int i = 0; i < img.getWidth(); i++)
					RGBE.float2re(bdata, img.getInternalData()[i + s], i + s * 2, img.getWidth());
		else
			for (int s = 0; s < img.getInternalData().length; s += img.getWidth() * 3)
				for (int i = 0; i < img.getWidth(); i++)
					RGBE.float2rgbe(bdata, img.getInternalData()[i * 3 + s], img.getInternalData()[i * 3 + s + 1],
							img.getInternalData()[i * 3 + s + 2], i + (s / 3) * 4, img.getWidth());
	...
	/**
	 * Reads an HDR image <f> into a HDRImage class structure. If <rgb> is not set,
	 * only the R channel of the HDR image will be read.
	 */
	public static HDRImage readHDR(File f, boolean rgb) throws IOException {
		...
		fis.close();
		return rgb ? new HDRImageRGB(width, height, outImage) : new HDRImageGrayscale(width, height, outImage);
